...
def get_angle_from_vector(direction: np.ndarray, base: np.ndarray, cos_margin: float = 1e-8,) -> np.ndarray:
    """
...
def get_vector_from_angle(angle: np.ndarray, base: np.ndarray) -> np.ndarray:
    """
    ...
    if norm_angle:
        vector = base.dot(np.hstack((np.cos(norm_angle), np.sin(norm_angle) * angle / norm_angle)))
    else:
        vector = base[:, 0]
    return vector
...
class UnitDirection(object):
    ...
    def from_angle(self, value: List[np.ndarray]) -> np.ndarray:
        """Update angle and reset 'equivalent' vector."""
        self._angle = np.array(value)
        self._vector = None
        return self

    def from_vector(self, value: List[np.ndarray]) -> None:
    ...
    def transform_to_base_old_try2(self, new_base: np.ndarray) -> None:
        ...
        for ii in range(dim):
            ...
            if ii > 0:
                ...
                if base_norm >= pi:
                    ...
                    dist_new_base = LA.norm(new_base_as_angle[:, ii] - new_base_as_angle[:, 0])

                    # Project accross origin
                    new_base_opposite_angle = (new_base_as_angle[:, ii] / base_norm * (base_norm - 2 * pi))

                    dist_new_base_opposite = LA.norm(new_base_opposite_angle - new_base_as_angle[:, 0])
        ...
        # Normalize vectors
        new_base_vecs = new_base_vecs / np.tile(LA.norm(new_base_vecs, axis=0), (new_base_vecs.shape[1], 1))
...
def get_directional_weighted_sum(
    ...
    if unit_directions is None:
        unit_directions = [UnitDirection(base).from_vector(directions[:, ii]) for ii in range(directions.shape[1])]
    else:
